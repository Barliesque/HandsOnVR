...
namespace Assets.Oculus.VR.Editor
{
	public class OVRPlatformTool : EditorWindow
	{
		...
		{
			var www = UnityWebRequest.Get(urlPlatformUtil);
			...
			while (!www.isDone && timer < timeOut)
			{
				...
				OVRPlatformTool.log = string.Format("Downloading.. {0:P1}", www.downloadProgress);
				SetDirtyOnGUIChange();
				yield return new WaitForSeconds(1f);
			}
			if (www.isDone)
			{
				System.IO.File.WriteAllBytes(dataPath, www.downloadHandler.data);
